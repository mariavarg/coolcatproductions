...
# CSRF token endpoint for JavaScript
...
@limiter.limit("3 per minute", methods=['POST'])  # Rate limit registrations
def register():
    if request.method == 'POST':
        try:
            users = load_data(app.config['USERS_FILE'])
...
if __name__ == '__main__':
    try:
        ...
        logger.critical(f"Failed to start server: {str(e)}")        os.makedirs('data', exist_ok=True)
        os.makedirs(app.config['COVERS_FOLDER'], exist_ok=True)
